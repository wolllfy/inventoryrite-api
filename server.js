...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .btn-bulk-increase { background: #16a34a; color: white; border: 0; }
        .btn-bulk-increase:hover { background: #15803d; }
        .btn-bulk-decrease { background: #dc2626; color: white; border: 0; }
        .btn-bulk-decrease:hover { background: #b91c1c; }
        .btn-bulk-clear { background: rgba(255,255,255,0.12); color: white; border: 1px solid rgba(255,255,255,0.18); }
        ...
        .stat-label { color: var(--muted); font-size: 12px; font-weight: 800; margin-bottom: 5px; }
        ...
        .empty strong { color: var(--text); display: block; margin-bottom: 5px; font-size: 15px; }
        ...
        .name-input:hover, .small-input:hover { border-color: #cbd5e1; }
        ...
        .setup-card h2 { margin: 0 0 10px; font-size: 30px; letter-spacing: -0.03em; }
        .setup-card p { color: var(--muted); line-height: 1.6; margin: 0 0 18px; }
        ...
        .modal p { margin: 0; color: var(--muted); line-height: 1.55; font-size: 14px; }
        ...
        .dev-link { color: var(--muted); text-decoration: none; font-size: 12px; }

        .sync-note { margin-top: 8px; color: var(--muted); font-size: 12px; font-weight: 800; }
        ...
        .merchant-control-left { color: white; min-width: 220px; }
        .merchant-control-title { font-size: 14px; font-weight: 900; line-height: 1.1; }
        .merchant-control-subtitle { margin-top: 4px; color: #cbd5e1; font-size: 11px; font-weight: 800; line-height: 1.3; }
...
<body>
    ...
    <main class="wrap">
                ...
                <div class="bulk-panel-header">
                    ...
                    <div class="bulk-controls">
                        <div>
                            ...
                            <input id="bulkPct" type="number" class="bulk-pct-input" value="10" min="0.01" max="9999" step="0.01" placeholder="10" />
                        </div>
                        <div style="display:flex;gap:8px;align-items:flex-end;padding-bottom:0;">
                            <button id="btnBulkIncrease" type="button" class="btn btn-small btn-bulk-increase">&#9650; Increase by %</button>
                            <button id="btnBulkDecrease" type="button" class="btn btn-small btn-bulk-decrease">&#9660; Decrease by %</button>
                            <button id="btnBulkClearPanel" type="button" class="btn btn-small btn-light">Clear Selection</button>
                        </div>
                    </div>
                </div>
        ...
        <div class="footer">
            InventoryRite for Clover &middot; Process Rite Inc &middot; <a class="dev-link" href="/dev">Developer tools</a>
        </div>
    ...
    (function () {
        ...
        function getToken() { return embeddedConnection.access_token || ""; }
        function getMerchantId() { return embeddedConnection.merchant_id || ""; }
        ...
        function formatDateFromClover(value) {
            if (!value) return "\u2014";
            try { return new Date(Number(value)).toLocaleString(); } catch (e) { return "\u2014"; }
        }
        ...
        function updateLastSyncNote() {
            ...
            if (note) note.textContent = "Last synced: " + new Date().toLocaleString();
        }

        function startBusy() { isBusy = true; }
        function stopBusy() { isBusy = false; }
        ...
        function getCostCents(itemId) { return Number(itemCosts[itemId] || 0); }
        ...
        function updateStats(items) {
            ...
            var totalCents = items.reduce(function (sum, item) { return sum + Number(item.price || 0); }, 0);
            ...
            var marginItems = items
                ...
                .map(function (item) {
                    ...
                    return { id: item.id || "", name: item.name || "Unnamed Product", price: price, cost: cost, margin: margin, profit: price - cost };
                });

            var belowCost = marginItems.filter(function (item) { return item.profit < 0; }).length;
            var avgMargin = marginItems.length ? marginItems.reduce(function (sum, item) { return sum + item.margin; }, 0) / marginItems.length : null;
        ...
        function syncBulkUI() {
            ...
            if (body) {
                ...
                rows.forEach(function (row) {
                    ...
                    if (selectedItemIds.has(id)) row.classList.add("row-selected");
                    else row.classList.remove("row-selected");
                });
            }
        }

        function clearSelection() { selectedItemIds.clear(); syncBulkUI(); }
        ...
        */
        ...
        async function runBulkPriceUpdate(direction) {
            ...
            if (selectedItemIds.size === 0) { showToast("Select at least one product to bulk update.", "error"); return; }
            ...
            openConfirm(
                ...
                async function () { await executeBulkUpdate(connection, selectedIds, pct, direction, dirLabel); }
            );
        }
        ...
        async function executeBulkUpdate(connection, selectedIds, pct, direction, dirLabel) {
            ...
            for (var i = 0; i < total; i++) {
                ...
                try {
                    await fetchJson(
                        ...
                        { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: item.name || "", price: newCents }) }
                    );
                    successCount++;
            ...
            if (failCount === 0) showToast("Bulk update complete: " + successCount + " price(s) " + dirLabel + " by " + pct + "%.", "success");
            else showToast("Bulk update: " + successCount + " succeeded, " + failCount + " failed.", failCount > 0 && successCount === 0 ? "error" : "info");
            clearSelection();
            stopBusy();
            await loadItems();
        }
        ...
        */
        ...
        function getItemSku(item) { return item.sku || item.code || item.productCode || ""; }
        ...
        function exportProductsCsv() {
            if (!loadedItems || loadedItems.length === 0) { showToast("Load inventory before exporting CSV.", "error"); return; }
            var rows = [];
            ...
            loadedItems.forEach(function (item) {
                ...
                rows.push([
                    csvEscape(item.id || ""), csvEscape(item.name || ""), csvEscape(getItemSku(item)),
                    csvEscape((price / 100).toFixed(2)), csvEscape((cost / 100).toFixed(2)),
                    csvEscape(margin === null ? "" : margin.toFixed(1)), csvEscape((profit / 100).toFixed(2)),
                    csvEscape(item.available === false ? "No" : "Yes"), csvEscape(item.hidden ? "Yes" : "No")
                ].join(","));
            });
        ...
        function showLowStock() {
            ...
            if (lowItems.length === 0) {
                openFeatureModal("Low Stock", "No low-stock items were found. If your Clover item response does not include quantity data yet, this will activate once inventory counts are available.", []);
            }
        ...
        function showProfitAlerts() {
            ...
            openFeatureModal("Profit Alerts", alerts.length ? (alerts.length + " product(s) need margin review.") : "No profit alerts right now. Good job \u2014 no below-cost or low-margin products were found.", rows);
        ...
        function showReorderPlanning() {
            ...
            var rows = lowItems.slice(0, 25).map(function (item) {
                var qty = getItemQuantity(item);
                return "<div><strong>" + escapeHtml(item.name || "Unnamed Product") + "</strong><span>Current quantity: " + escapeHtml(qty === null ? "Unknown" : qty) + " \u00b7 Suggested reorder: review supplier stock</span></div><div><span>Plan</span></div>";
            });
            openFeatureModal("Smart Reorder Planning", lowItems.length ? "These items may need reorder attention." : "No reorder suggestions yet. This becomes stronger after quantity and sales velocity data are available.", rows);
        }

        function showPriceRules() {
            openFeatureModal("Price Rules", "Coming next: create rules like increase all drinks by 8%, round prices to .99, or protect minimum margins before applying changes.", [
                "<div><strong>Example Rule</strong><span>Increase selected category by 8%</span></div><div><span>Coming next</span></div>",
                "<div><strong>Margin Protection</strong><span>Prevent products from selling below cost</span></div><div><span>Coming next</span></div>"
            ]);
        }

        function showActivityLog() {
            openFeatureModal("Activity Log", "Coming next: every price change, cost update, delete, and bulk update will appear here with time and merchant context.", [
                "<div><strong>Product Updated</strong><span>Track old price, new price, and time</span></div><div><span>Planned</span></div>",
                "<div><strong>Undo Support</strong><span>Use activity history to reverse mistakes safely</span></div><div><span>Planned</span></div>"
            ]);
        }

        function importCsvClicked() {
            openFeatureModal("Import CSV", "CSV import is staged safely. Next step is mapping columns before making bulk changes to Clover, so merchants do not accidentally overwrite products.", [
                "<div><strong>Safe Import Flow</strong><span>Upload CSV \u2192 preview changes \u2192 confirm update</span></div><div><span>Next build</span></div>"
            ]);
        }
        ...
        */
        ...
        function renderItems(items) {
            ...
            if (!items || !items.length) {
                body.innerHTML = '<tr><td colspan="6" class="empty"><strong>No Clover products found.</strong>Click Add Product to create your first item.</td></tr>';
                syncBulkUI();
                return;
            }

            if (!filtered.length) {
                body.innerHTML = '<tr><td colspan="6" class="empty"><strong>No matching products found.</strong>Try a different product name, SKU, or Clover ID.</td></tr>';
                syncBulkUI();
                return;
            }
            ...
            filtered.forEach(function (item) {
                ...
                if (lastUpdatedItemId && itemId === lastUpdatedItemId) row.className = "row-updated";
                else if (isBulkUpdated) row.className = "row-bulk-updated";

                if (profitCents < 0 && costCents > 0) row.classList.add("row-below-cost");
                ...
                row.innerHTML =
                    ...
                    "<td><input class='small-input' data-cost-for='" + escapeHtml(itemId) + "' value='" + escapeHtml(costDollars) + "' title='Your cost of goods.' /></td>" +
            ...
            if (lastUpdatedItemId) {
                setTimeout(function () { lastUpdatedItemId = ""; renderItems(loadedItems); }, 1400);
            }
            if (bulkUpdatedItemIds.length > 0) {
                setTimeout(function () { bulkUpdatedItemIds = []; renderItems(loadedItems); }, 1800);
            }

            var checkboxes = body.querySelectorAll("input[type='checkbox'][data-item-id]");
            checkboxes.forEach(function (cb) {
                cb.addEventListener("change", function (e) {
                    toggleItemSelection(e.target.getAttribute("data-item-id"), e.target.checked);
                });
            });

            var costInputs = body.querySelectorAll("input[data-cost-for]");
            costInputs.forEach(function (input) {
                input.addEventListener("blur", function (e) { saveItemCost(e.target.getAttribute("data-cost-for"), e.target.value); });
        ...
        async function fetchJson(url, options) {
            ...
            var timeoutId = setTimeout(function () { try { controller.abort(); } catch (e) {} }, 25000);
            ...
            try {
                ...
                if (fetchError && fetchError.name === "AbortError") throw new Error("Request timed out. Please refresh inventory again.");
                throw fetchError;
            } finally {
                clearTimeout(timeoutId);
            }
            var data = null;
            try { data = await response.json(); } catch (jsonError) { data = { success: false, message: "Server returned a non-JSON response." }; }
            if (!response.ok) {
        ...
        async function saveItemCost(itemId, value) {
            try {
                ...
                if (costCents === null) { showToast("Cost must be a valid dollar amount.", "error"); renderItems(loadedItems); return; }
                var savedCost = await fetchJson(
                    "/item-cost/" + encodeURIComponent(itemId) +
                    "?token=" + encodeURIComponent(connection.token) +
                    "&merchantId=" + encodeURIComponent(connection.merchantId),
                    { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ costCents: costCents }) }
                );
        ...
        async function loadItems() {
            ...
            try {
                ...
                var data = await fetchJson("/clover-items?token=" + encodeURIComponent(connection.token) + "&merchantId=" + encodeURIComponent(connection.merchantId));
                var costData = await fetchJson("/item-costs?token=" + encodeURIComponent(connection.token) + "&merchantId=" + encodeURIComponent(connection.merchantId));
        ...
        async function createItem() {
            ...
            try {
                ...
                if (!name) { showToast("Product name cannot be empty.", "error"); return; }
                if (Number.isNaN(Number(price)) || Number(price) < 0) { showToast("Price cents must be a valid positive number.", "error"); return; }
                startBusy();
                setButtonText("btnRefreshInventory", "Refreshing...");
                setButtonText("btnRefreshInventoryTop", "Refreshing...");
                var data = await fetchJson(
                    "/clover-create-item?token=" + encodeURIComponent(connection.token) + "&merchantId=" + encodeURIComponent(connection.merchantId),
                    { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: name, price: Number(price) }) }
                );
        ...
        async function updateItem(itemId) {
            ...
            try {
                ...
                if (!name) { showToast("Product name cannot be empty.", "error"); return; }
                if (priceCents === null) { showToast("Price must be a valid dollar amount.", "error"); return; }
                startBusy();
                await fetchJson(
                    "/clover-update-item/" + encodeURIComponent(itemId) +
                    "?token=" + encodeURIComponent(connection.token) + "&merchantId=" + encodeURIComponent(connection.merchantId),
                    { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: name, price: priceCents }) }
                );
        ...
        function toggleAddPanel() {
            ...
            if (addPanel.classList.contains("show")) { var itemName = byId("itemName"); if (itemName) itemName.focus(); }
        }
        ...
        function toggleBulkPanel() {
            ...
            if (bulkPanel.classList.contains("show")) { var pctInput = byId("bulkPct"); if (pctInput) pctInput.focus(); }
        }

        /*
        |------------------------------------------------------------------
        | EVENT BINDINGS — deferred so all DOM elements are guaranteed present
        |------------------------------------------------------------------
        */
        ...
        function attachBindings() {
            ...
            bind("selectAllCheckbox", "change", function (e) { selectAllVisible(e.target.checked); });
            ...
            bind("btnBulkClearPanel", "click", function () { clearSelection(); renderItems(loadedItems); });

            var confirmModal = byId("confirmModal");
            if (confirmModal) {
        ...
        // Guarantee DOM is ready before attaching any listeners
...
app.get("/clover-merchant", async (req, res) => {
    try {
        ...
        if (!accessToken || !merchantId) {
            return res.status(400).json({ success: false, message: "Missing token or merchantId." });
        }
        ...
        res.json({ success: true, message: "Clover merchant info loaded successfully", data: merchantResponse.data });
    } catch (error) {
        console.error("Clover Merchant Error:", error.response?.data || error.message);
        const cloverError = getCloverError(error);
        res.status(cloverError.status).json({ success: false, message: "Failed to load Clover merchant info", error: cloverError.data });
    }
});
...
app.get("/clover-items", async (req, res) => {
    try {
        ...
        res.json({ success: true, message: "Clover inventory items loaded successfully", data: itemsResponse.data });
    } catch (error) {
        console.error("Clover Items Error:", error.response?.data || error.message);
        const cloverError = getCloverError(error);
        res.status(cloverError.status).json({ success: false, message: "Failed to load Clover inventory items", error: cloverError.data });
    }
});
...
app.post("/clover-create-item", async (req, res) => {
    try {
        ...
        if (!itemName || !isValidMoneyCents(itemPrice)) {
            return res.status(400).json({ success: false, message: "Invalid item name or price." });
        }

        const createResponse = await cloverApi.post(
            `${CLOVER_API_BASE_URL}/v3/merchants/${merchantId}/items`,
            { name: itemName, price: itemPrice, priceType: "FIXED", available: true, hidden: false, isRevenue: true },
            { headers: cloverHeaders(accessToken) }
        );

        res.json({ success: true, message: "Clover item created successfully", data: createResponse.data });
    } catch (error) {
        console.error("Clover Create Item Error:", error.response?.data || error.message);
        const cloverError = getCloverError(error);
        res.status(cloverError.status).json({ success: false, message: "Failed to create Clover item", error: cloverError.data });
    }
});
...
app.post("/clover-update-item/:itemId", async (req, res) => {
    try {
        ...
        res.json({ success: true, message: "Clover item updated successfully", data: updateResponse.data });
    } catch (error) {
        console.error("Clover Update Item Error:", error.response?.data || error.message);
        const cloverError = getCloverError(error);
        res.status(cloverError.status).json({ success: false, message: "Failed to update Clover item", error: cloverError.data });
    }
});
...
app.post("/clover-delete-item/:itemId", async (req, res) => {
    try {
        ...
        res.json({ success: true, message: "Clover item deleted successfully", data: deleteResponse.data || {} });
    } catch (error) {
        console.error("Clover Delete Item Error:", error.response?.data || error.message);
        const cloverError = getCloverError(error);
        res.status(cloverError.status).json({ success: false, message: "Failed to delete Clover item", error: cloverError.data });
    }
});
...
app.get("/item-costs", async (req, res) => {
    try {
        ...
        res.json({ success: true, databaseEnabled: USE_DATABASE, costs });
    } catch (error) {
        console.error("Item Costs Error:", error.message);
        res.status(500).json({ success: false, message: "Failed to load item costs.", error: error.message });
    }
});
...
app.post("/item-cost/:itemId", async (req, res) => {
    try {
        ...
        if (!isValidMoneyCents(costCents)) {
            return res.status(400).json({ success: false, message: "Invalid cost amount." });
        }
        ...
        res.json({
            success: true,
            message: cloverCostSynced ? "Item cost saved and synced to Clover." : "Item cost saved in InventoryRite. Clover cost sync was not confirmed.",
            databaseEnabled: USE_DATABASE,
            itemId,
        ...
        res.status(500).json({ success: false, message: "Failed to save item cost.", error: error.message });
    }
});
