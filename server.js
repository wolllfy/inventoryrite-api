...
/).filter(function (line) { return String(line || "").trim(); });
                    ...
                    openFeatureModal(
                        "CSV File Selected",
                        "InventoryRite opened your CSV safely. No Clover prices were changed automatically.",
                        [
                            "<div><strong>" + escapeHtml(file.name || "CSV file") + "</strong><span>" + rowCount + " product row(s) detected. Use Export CSV as the safest template before importing live updates.</span></div><div><span>Preview</span></div>",
                            "<div><strong>Protected Pricing</strong><span>InventoryRite never changes Clover prices without confirmation.</span></div><div><span>Safe</span></div>"
                        ]
                    );
                    logActivity("Import CSV", "CSV file selected: " + (file.name || "CSV file") + " with " + rowCount + " row(s).", "Preview");
                    showToast("CSV file opened safely. No Clover prices changed.", "success");
                } catch (err) {
                    showToast("Unable to read CSV file. Please export a fresh CSV and try again.", "error");
                } finally {
                    var input = byId("csvImportInput");
            ...
            reader.onerror = function () {
                showToast("Unable to read CSV file. Please try again.", "error");
            };
